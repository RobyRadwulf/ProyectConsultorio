...
def dashboard(request):
    ...
    return render(request, 'consultorio/dashboard.html', {'all_clients': all_clients, 'all_user': all_user,'page_obj': page_obj})
...
def assistant_register(request):
    if request.method == 'POST':
        ...
        if assistant_form.is_valid():
            ...
            new_user.save()
            return render(request, 'consultorio/register_done.html', {'new_user': new_user})
    else:
        assistant_form = AssistantRegistrationForm()
        return render(request, 'consultorio/register_assistant.html', {'assistant_form': assistant_form})
...
def nutritionist_register(request):
    if request.method == 'POST':
        ...
        return render(request, 'consultorio/register_nutritionist.html', {'nutritionist_form': nutritionist_form})
...
def client_register(request):
    if request.method == 'POST':
        ...
        if client_form.is_valid():
            ...
            new_client.save()
            return render(request, 'consultorio/register_client_done.html', {'new_client': new_client})
    else:
        client_form = ClientRegistrationForm()
        return render(request, 'consultorio/register_client.html', {'client_form': client_form})
...
def make_consult(request):
    if request.method == 'POST':
        ...
        if consult_form.is_valid():
            ...
            new_consult.save()
            return render(request, 'consultorio/consult_finished.html', {'new_consult': new_consult})
    else:
        consult_form = MakeConsult()
        return render(request, 'consultorio/consult.html', {'consult_form': consult_form})

@login_required
def patient_details(request, client_id=None):
    client = get_object_or_404(patient, pk=client_id)
    all_client = patient.objects.all()
...
def patient_progress(request):
    ...
    client = get_object_or_404(patient, id=client_id)
    ...
    return render(request, 'consultorio/patient_progress.html', {'client': client, 'consult': consult,
